...
class Encoder(nn.Module):
    ...
    def forward(self, x, encoder_outs=None, decoder_outs=None):
        ...
        if (encoder_outs is not None) and (decoder_outs is not None):
            he2 = self.csff_enc2(encoder_outs[1])
            hd2 = self.csff_dec2(decoder_outs[1])
            he2 = torch.nn.functional.interpolate(he2, size=(enc2.size(2), enc2.size(3)), mode='bicubic')
            hd2 = torch.nn.functional.interpolate(hd2, size=(enc2.size(2), enc2.size(3)), mode='bicubic')
            enc2 = enc2 + he2 + hd2
        ...
        if (encoder_outs is not None) and (decoder_outs is not None):
            he3 = self.csff_enc3(encoder_outs[2])
            he3 = torch.nn.functional.interpolate(he3, size=(enc3.size(2), enc3.size(3)), mode='bicubic')
            hd3 = self.csff_dec3(decoder_outs[2])
            hd3 = torch.nn.functional.interpolate(hd3, size=(enc3.size(2), enc3.size(3)), mode='bicubic')
            enc3 = enc3 + he3 + hd3
...
class SkipUpSample(nn.Module):
    ...
    def forward(self, x, y):
        x = self.up(x)
        x = torch.nn.functional.interpolate(x, size=(y.size(2), y.size(3)), mode='bicubic')
        x = x + y
        return x
...
class ORSNet(nn.Module):
    ...
    def forward(self, x, encoder_outs, decoder_outs):
        ...
        x = self.orb2(x)
        t = self.conv_enc2(self.up_enc1(encoder_outs[1])) + self.conv_dec2(self.up_dec1(decoder_outs[1]))
        t = torch.nn.functional.interpolate(t, size=(x.size(2), x.size(3)), mode='bicubic')
        x = x + t

        x = self.orb3(x)
        t1 = self.conv_enc3(self.up_enc2(encoder_outs[2])) + self.conv_dec3(self.up_dec2(decoder_outs[2]))
        t1 = torch.nn.functional.interpolate(t1, size=(x.size(2), x.size(3)), mode='bicubic')
        x = x + t1
